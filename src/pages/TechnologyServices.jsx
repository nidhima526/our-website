          ...
          <div className="max-w-[95rem] mx-auto px-6 md:px-12 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 auto-rows-min">
              {additionalServices.map((category, idx) => (
                <div 
                  key={idx} 
                  className="bento-card relative group bg-[#0a0a0a] rounded-[2rem] border border-white/5 hover:border-yellow-500/30 transition-all duration-700 overflow-hidden flex flex-col shadow-2xl hover:shadow-[0_0_50px_rgba(202,138,4,0.15)] h-full"
                >
                  ...
                  <div className="relative z-10 flex flex-col flex-1 p-8 bg-white/[0.02]">
                    ...
                    <ul className="space-y-4 flex-1">
                      {(expandedCategories[idx] ? category.items : category.items.slice(0, 6)).map((item, i) => (
                      ...
                      {category.items.length > 6 && (
                        <li 
                          ...
                          className="text-yellow-600/80 hover:text-yellow-500 cursor-pointer font-bold text-xs mt-6 tracking-widest uppercase pl-4 border-l-2 border-yellow-600/30 transition-colors"
                        >
                          {expandedCategories[idx] ? "- Show Less Capabilities" : `+ Explore ${category.items.length - 6} More Capabilities`}
                        </li>
                      )}
                    </ul>
                  </div>
